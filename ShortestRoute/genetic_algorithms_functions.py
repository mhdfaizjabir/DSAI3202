...
def calculate_fitness(route,
    ...
    # Loop through each consecutive pair of nodes in the route
    for i in range(len(route) - 1):
        dist = distance_matrix[route[i], route[i + 1]]  # Get the distance
        if dist == 100000:  # Check for infeasible route
            return -1e6  # Apply large negative penalty
        total_distance += dist  # Add distance to total

    # Add distance back to the depot (start node)
    dist_to_depot = distance_matrix[route[-1], route[0]]
    if dist_to_depot == 100000:
        return -1e6  # Apply large negative penalty for infeasible route
    total_distance += dist_to_depot

    return -total_distance  # Return negative value for minimization
...
def select_in_tournament(population,
    ...
    """
    selected = []  # Initialize the list of selected individuals

    for _ in range(number_tournaments):
        # Randomly select individuals for the tournament
        indices = np.random.choice(len(population), tournament_size, replace=False)
        tournament_individuals = [population[idx] for idx in indices]
        tournament_scores = [scores[idx] for idx in indices]

        # Select the best individual in the tournament
        best_idx = np.argmax(tournament_scores)  # Max because scores are negative distances
